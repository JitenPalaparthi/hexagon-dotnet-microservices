...
var subscription = Environment.GetEnvironmentVariable("SUBSCRIPTION__NAME") ?? "sub-a";
...
var options = new ServiceBusSessionProcessorOptions
{
    ...
    MaxConcurrentCallsPerSession = 1, // preserve FIFO per session
    PrefetchCount = Math.Max(0, prefetch),
    AutoCompleteMessages = false,
    SessionIdleTimeout = TimeSpan.FromSeconds(30)
};

var processor = client.CreateSessionProcessor(topic, subscription, options);

processor.ProcessMessageAsync += async args =>
{
    var sid = args.SessionId;
    var body = args.Message.Body.ToString();
    Console.WriteLine($"[{DateTime.Now:HH:mm:ss}] SUB={subscription} SID={sid} SEQ={args.Message.SequenceNumber} BODY={body}");
...
{
    Console.WriteLine($"[ASB ERROR] SUB={subscription} Source={args.ErrorSource} Entity={args.EntityPath} Ex={args.Exception.Message}");
    return Task.CompletedTask;
};

Console.WriteLine($"Starting session processor on '{topic}/Subscriptions/{subscription}' (sessions={options.MaxConcurrentSessions}, prefetch={options.PrefetchCount}, failStep={failStep?.ToString() ?? "none"})");
